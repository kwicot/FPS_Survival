...
namespace _Core.Scripts.WorldGeneratorCore
{
    public static class Noise
    {
        ...
        {
            //Debug.Log($"Generate heightMap size {mapHeight} {mapHeight}");
        ...
        public static int[,] GenerateBiomeMap(int mapWidth, int mapHeight, int seed, int biomesGrid, int biomesNum,
            float noiseMult, float noiseDist)
        {
                ...
                for (int y = 0; y < mapHeight; y++)
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        for (int j = 0; j < 4; j++)
                        {
                            ...
                            int dist =
                                ((gridX + i) * biomesGrid + biomeX - x) * ((gridX + i) * biomesGrid + biomeX - x) +
                                ((gridY + j) * biomesGrid + biomeY - y) * ((gridY + j) * biomesGrid + biomeY - y);

                            dist += (int) (Mathf.PerlinNoise(
                                noiseDist * ((gridX + i) * biomesGrid + biomeX - x + offsetX) / 100f,
